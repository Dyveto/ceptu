...
export default function ReportsTab() {
  ...
  // Filtros dinámicos requeridos por la API (Inician con valores por defecto del año actual)
  ...
  const loadAnalyticsData = async () => {
    ...
    setError('');
    try {
      // Formatear los query params exactamente como los espera Spring Boot
      ...
      // Consultar los 4 endpoints en paralelo para máxima eficiencia
      ...
      setLowStock(lowRes || []);
      // Si el backend devuelve un objeto o un número crudo, nos aseguramos de capturarlo bien
      setMonthlyIncome(incomeRes?.totalIncome || incomeRes || 0);
  ...
  // Cargar datos al montar el componente o cuando cambien los filtros
  useEffect(() => {
    loadAnalyticsData();
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        ...
        <div style={{ flex: 1, background: lowStock.length > 0 ? '#ffebee' : '#e8f5e9', borderLeft: `5px solid ${lowStock.length > 0 ? '#f44336' : '#4caf50'}`, padding: '1.2rem', borderRadius: '6px' }}>
          ...
          <h2 style={{ margin: '0.5rem 0 0 0', color: lowStock.length > 0 ? '#c62828' : '#1b5e20' }}>
            {lowStock.length} {lowStock.length === 1 ? 'Producto crítico' : 'Productos estables'}
          </h2>
        </div>
      </div>
              ...
              {bestSelling.length === 0 ? (
                ...
                bestSelling.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ color: '#0076ff', fontWeight: 'bold' }}>{item.totalQuantity || item.soldUnits} unds</td>
                  </tr>
                ))
              )}
            </tbody>
            ...
            <tbody>
              {topCustomers.length === 0 ? (
                ...
                topCustomers.map((item, index) => (
                  <tr key={index}>
                    <td>{item.customerName || `${item.firstName} ${item.lastName}`}</td>
                    <td style={{ color: '#34c759', fontWeight: 'bold' }}>${item.totalSpent?.toLocaleString()}</td>
                  </tr>
                ))
              )}
            </tbody>
